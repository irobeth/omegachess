package net.omegachess.rules
{
	...
	public class CanCastleQueenside implements IOmegaChessRule
	{
		...
		public function satisfies(board:Board) : Boolean {
			Debug.show(Debug.FUNCTION, "CanCastleQueenside", "");
			trace(board.toOCPN());
			/*
			...
			*/
			...
			if(board.movingAlignment == Piece.ALIGNMENT_BLACK) {
				...
				trace("King at " + kingCell.rank + ", " + kingCell.file);
				king 		= board.blackKing.piece;				
				//black kingside rook is at B9 (9,1)
				rookCell 	= board.getCellAt(10, 2);
				rook 		= rookCell.piece;
				...
				//white kingside rook is at B1 (1,1)
				rookCell 	= board.getCellAt(1, 2);
				rook 		= rookCell.piece;
			}
			...
			if(!(rook is Rook)) {
				Debug.show(Debug.INFO, "CanCastleQueenside", "Failing to satisfy because the rook cell wasn't a rook.");
				Debug.show(Debug.INFO, "", rookCell.toString());
				return false;	
			}
			if(rook.alignment != board.movingAlignment) {
				Debug.show(Debug.INFO, "CanCastleQueenside", "Failing to satisfy because the rook wasn't owned by the same alignment.");
				return false;	
			}
			
			if(king.hasMoved == true) {
				Debug.show(Debug.INFO, "CanCastleQueenside", "Failing to satisfy because the rook king had moved.");
				return false;	
			}			
			if(rook.hasMoved == true) {
				Debug.show(Debug.INFO, "CanCastleQueenside", "Failing to satisfy because the rook had moved.");
				return false;	
			}
			...
			for(var file : Number = kingCell.file - 1; file > rookCell.file; file --) {
				if(!(board.getCellAt(kingCell.rank, file).piece is Hole)) {
					Debug.show(Debug.INFO, "CanCastleQueenside", "Failing to satisfy because " + board.getCellAt(kingCell.rank, file).toACN() + " is occupied by " + board.getCellAt(kingCell.rank, file).piece.acn());
					return false;	
				}
				if(threatbits.getBitAt(kingCell.rank, file) != 0) {
					Debug.show(Debug.INFO, "CanCastleQueenside", "Failing to satisfy because " + board.getCellAt(kingCell.rank, file).toACN() + " is threatened.");
					return false;	
				}
			} 
